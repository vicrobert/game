...
void print_s(const char * s, int h, int v)
{
	printf("\033[%d;%dH%s\n", v, h << 1, s);
}

void print_i(int i, int h, int v)
{
	printf("\033[%d;%dH%d\n", v, h << 1, i);
}
...
int put_egg(int count)
{
	...
	static int failure_time = 0;
	struct body_slice_type *pslice;

	srand(time(0));
	pos = (STAGE_HEIGHT - 2) * (STAGE_WIDTH - 2) * (rand() / (RAND_MAX + 1.0));
	x = (pos % (STAGE_WIDTH - 2)) + STAGE_ORG_X + 1;
	y = (pos / (STAGE_WIDTH - 2)) + STAGE_ORG_Y + 1;
	i = y - STAGE_ORG_Y;
	j = x - STAGE_ORG_X;
